...
import { FactvAjoutProduitPage } from "../factv-ajout-produit/factv-ajout-produit";
import { FactvEditProduitPage } from "../factv-edit-produit/factv-edit-produit";
import { FactvRemisePage } from "../factv-remise/factv-remise";
...
@IonicPage()
@Component({
  selector: 'page-factv-data',
  templateUrl: 'factv-data.html',
})
export class FactvDataPage {
  $size = 15;
  $skip = 0;
  ...
  infoProformaGle() {
    ...
    this.cmdApi.getTypeVentSelCmd(this.numBon, this.serverURL).subscribe(
      res => {
        ...
        this.NUMCDE = this.proforma_data.NUMCDE;
        ...
        if (this.proforma_data.AVOIR == null) {
          this.mod_dev.AVOIR = false;
        } else {
          if (this.proforma_data.AVOIR == "OUI") {
            this.mod_dev.AVOIR = true;
          } else {
            this.mod_dev.AVOIR = false;
          }
        }
  ...
  editer($NUMBON, $LIGNE, $ORDRE, $VALIDE) {
    console.log('ligne:', $NUMBON, $LIGNE)
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas editer ce produit parce que la facture est validée !',
        buttons: ['OK']
      });
      alert.present();

    } else {
      this.dprod = { $NUMBON: $NUMBON, $LIGNE: $LIGNE, $ORDRE: $ORDRE };
      this.navCtrl.push(FactvEditProduitPage, this.dprod);
  ...
  calcul_rem_details() {
    if (this.mod_dev.remisePrc == 0 && this.mod_dev.remiseTaux > 0) {
      //raf //taux c depuis la bdd et le pourcetage c 0
    } else {
      this.mod_dev.remiseTaux = (this.totaux_bht * this.mod_dev.remisePrc) / 100;
    }
    this.TotalNetHt = (this.totaux_bht - this.mod_dev.remiseTaux);
    if (this.totaux_bht != 0) {
      this.taux_tva_g_r = this.taux_tva_g - (this.taux_tva_g * this.mod_dev.remiseTaux / this.totaux_bht);
    } else {
      this.taux_tva_g_r = 0;
    }
    ...
    if (this.mod_dev.AVOIR == true) {
      this.remise.taux = this.remise.taux * -1;
  ...
  ajouter_des_prod_d($NUMBON, $VALIDE) {
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas ajouter des produits parce que la facture est validée !',
        buttons: ['OK']
      });
      alert.present();
    } else {
      this.navCtrl.push(FactvAjoutProduitPage, $NUMBON);
    }
  };
  ...
  ajt_remise_globle($c_proforma, $totaux_bht, $valide) {
    if ($valide) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas appliquer une remise globale parce que la facture est validée !',
        buttons: ['OK']
      });
      alert.present();
    } else {
      ...
      this.navCtrl.push(FactvRemisePage, this.data_remise);
    }
  };
  ...
  details_globale() {
    this.calcul_rem_details();
    let alert = this.alertCtrl.create({
      title: 'Détails facture N°' + this.numBon,
      subTitle: '' +
  ...
  supprimer($NUMBON, $ORDRE, $VALIDE) {
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas supprimer ce produit parce que la facture est validée !",
        buttons: ['Annuler']
      });
      alert.present();
      ...
      let alert = this.alertCtrl.create({
        ...
        buttons: [
          ...
          },
          {
            text: 'Supprimer',
            handler: () => {
              this.cmdApi.delete_produit_facturev($NUMBON, $ORDRE, this.serverURL).subscribe(
                res => {
                  this.data = res;
                  this.refresh_list_DPRO();
                  let alert = this.alertCtrl.create({
  ...
  modifier_devis($mod_dev) {
    ...
    if ($mod_dev.VALIDE) {
      ...
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas modifier une facture validée !",
        buttons: ['OK']
      });
      alert.present();
      ...
      this.cmdApi.update_info_facturev($mod_dev, this.serverURL).subscribe(
        res => {
          this.data = res;
          let alert = this.alertCtrl.create({
            title: "Information",
      ...
      buttons: [
        {
          ...
          text: 'Ok', handler: () => {
            this.cmdApi.getTypeVentSelCmd(this.numBon, this.serverURL).subscribe(
              res => {
                this.data = res;
                this.valide_bdd_r = this.data.return[0].AVOIR;
                if (this.valide_bdd_r == "OUI") {
                  this.cmdApi.setAvoirNon(this.numBon, this.serverURL).subscribe(
                    res => {
                      let alert = this.alertCtrl.create({
                        title: "Information",
                        subTitle: res['msg'],
                  ...
                  this.cmdApi.setAvoirOui(this.numBon, this.serverURL).subscribe(
                    resp => {
                      let alert = this.alertCtrl.create({
                        title: "Information",
                        subTitle: resp['msg'],
