...
@IonicPage()
@Component({
  selector: 'page-devis-ajout-produit',
  templateUrl: 'devis-ajout-produit.html',
})
export class DevisAjoutProduitPage {
  users = []
  $size = 15;
  $skip = 0;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public devisApi: DevisProvider,
    public produitsApi: ProduitsProvider, public loadingCtrl: LoadingController,
    public alertCtrl: AlertController, public storage: Storage, public barcodeScanner: BarcodeScanner) {
    this.prod_aj = {
      "NUMBON": null,
      "d_prod": null,
      "LIGNE": null,
    ...
    this.storage.get('ip_adr').then((val) => {
      ...
      this.getOrdreDevisProd(this.prod_aj.NUMBON);
    });
  ...
  getOrdreDevisProd($nbon) {
    this.devisApi.d_proforma($nbon, this.serverURL).subscribe(
      res => {
        this.data = res;
  ...
  async localgetTypeVentSelDevis($c_prod, $qtes, $remise, data$) {
    await this.devisApi.getTypeVentSelDevis(this.prod_aj.NUMBON, this.serverURL).subscribe(
      resp => {
        this.dataresp = resp;
  ...
  async postProdDevis(devis$) {
    await this.devisApi.insert_produit_to_devis(devis$, this.serverURL)
      .subscribe(res => {
        let alert = this.alertCtrl.create({
          title: "Information",
          subTitle: res['msg'],
        ...
        alert.present();
       // this.navCtrl.push(DevisDataPage, devis$.NUMBON);
      }, (err) => { console.log(err); });
  ...
  ajouterdetails($prod_aj) {
    if ($prod_aj.d_prod == null) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "Veuillez saisir un produit à ajouter à cette proforma !",
        buttons: ['OK']
      });
      alert.present();
