...
@IonicPage()
@Component({
  selector: 'page-devis-edit-produit',
  templateUrl: 'devis-edit-produit.html',
})
export class DevisEditProduitPage {
  users = []
  $size = 15;
  $skip = 0;
...
EQUIV:any;
  ...
  async getdetailsProduit(){
    ...
    await   this.devisApi.getProduitDevis( this.prod_aj.NUMBON, 
 ...
 async postProdDevis(devis$){
    await this.devisApi.updateProduitDevis(devis$ , this.serverURL)
      .subscribe(res => {
        let alert = this.alertCtrl.create({
          title: "Information",
          subTitle: res['msg'],
        ...
        this.devisdata.refresh_list_DPRO();
      }, (err) => { console.log(err); });
  }
