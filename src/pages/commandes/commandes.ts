...
import { CommandesAjoutPage } from '../commandes-ajout/commandes-ajout';
...
export class CommandesPage {
  ...
  constructor(public navCtrl: NavController,
    public restApi: CommandesProvider,
    ...
    this.storage.get('ip_adr').then((val) => {
      this.serverURL = val;
      //Récuperer les informations de l'enete qui vont etre utilisé dans le PDF
      this.getEntete();
  ...
  /*1- RECUPERER LA LISTE DES PRODUITS DU BON DE commande
              ...
              res => {
                ...
                doc.html(pdfBlock, {
                  callback: (doc) => {
                    ...
                    // doc.save('Commande_'+item.NUMBON);
                ...
                if (window.hasOwnProperty('cordova')) {
                  ...
                  domtoimage.toPng(pdfBlock, options).then((fileUrl) => {
                    ...
                    const fileName = "Commande" + item.NUMBON + ".pdf";
  ...
  goToAjoutCmd() {
    this.navCtrl.push(CommandesAjoutPage);
  }
  goToCmdData($numbon,$c_client) {
    this.navCtrl.push(CommandesDataPage, {$numbon:$numbon,$c_client:$c_client});
  }
  ...
  convert_list($numbon,$c_client) {
    ...
    alert.setTitle('Convertir la commande n°' + $numbon + ' en ?');
    ...
    alert.addButton({
      ...
      handler: data => {
        //var
        this.$nb_from = $numbon;
        this.$table_from = "COMMANDC";
        this.$table_detail_from = "D_COMC";
  ...
  async convertPost($$nb_from, $$table_from, $$table_detail_from, $$table_to, $$table_detail_to) {
    ...
    await this.conversionApi.convertCommandeClient(this.$data_conv, this.serverURL).subscribe(
      res => {
        this.data = res;
        console.log(res);
        let alert = this.alertCtrl.create({
