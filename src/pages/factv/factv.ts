...
import { FactvAjoutPage } from "../factv-ajout/factv-ajout";
...
@Component({
  selector: 'page-factv',
  templateUrl: 'factv.html',
})
export class FactvPage {
  data: any;
  users = [];
  $size = 10;
  $skip = 0;
  ...
  constructor(public navCtrl: NavController,
    public restApi: FactureVenteProvider,
...
ionViewWillEnter(){
  ...
  /*1- RECUPERER LA LISTE DES PRODUITS DE LA FACTURE
    ...
    //On récupère le mode de paiement qu'a utilisé le clients ainsi que les informations sur le reglement
    this.restApi.get_Reglement_by_NumBon(item.NUMFACT, this.serverURL)
      .subscribe(
        res => {
          ...
          this.restApi.get_d_cmd_ParNumBon(item.NUMFACT, this.serverURL)
            .subscribe(
              res => {
                this.DetailBon = res;
                ...
                doc.html(pdfBlock, {
                  callback: (doc) => {
                    ...
                    //Ajouter La date et le numéro de la facture
                    for (var i = 1; i <= totalPages; i++) {
                      doc.setPage(i);
                      doc.setFontSize(15);
                      doc.setTextColor(10);
                      ...
                      /*On vérifie si la facture est un avoir */
                      if (item.AVOIR == 'OUI') {
                        doc.text('Avoir N° : ', 20, 37);

                      }
                      else { doc.text('Facture N° : ', 20, 37); }

                      doc.text(item.NUMFACT + '   /   ' + this.ExerciceEnCours, 70, 37);
                    }
                    ...
                    // doc.save('Facture_' + item.NUMFACT);
                ...
                if (window.hasOwnProperty('cordova')) {
                  ...
                  domtoimage.toPng(pdfBlock).then((fileUrl) => {
                    ...
                    const fileName = "Facture" + item.NUMFACT + ".pdf";
  ...
  chargerDataApi($skip, $size) {
    this.restApi.getfacturev($skip, $size, this.serverURL)
      .subscribe(
        res => {
          this.data = res;
  ...
  goToAjoutCmd() {
    this.navCtrl.push(FactvAjoutPage);
  }

  goToCmdData($numbon,$c_client) {
    console.log('c_client',$c_client);
    this.navCtrl.push(FactvDataPage,{$numbon:$numbon,$c_client:$c_client});
  }
  ...
  convert_list($numbon,$c_client) {
    ...
    alert.setTitle('Convertir la facture n°' + $numbon + ' en ?');
    ...
    alert.addButton({
      ...
      handler: data => {
        //var
        this.$nb_from = $numbon;
        this.$table_from = "FACTURE";
        this.$table_detail_from = "D_FACT";
  ...
  async convertPost($$nb_from, $$table_from, $$table_detail_from, $$table_to, $$table_detail_to) {
    ...
    await this.conversionApi.convertFacture(this.$data_conv, this.serverURL).subscribe(
      res => {
        this.data = res;
        console.log(res);
        let alert = this.alertCtrl.create({
