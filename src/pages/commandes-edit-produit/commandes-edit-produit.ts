...
@IonicPage()
@Component({
  selector: 'page-commandes-edit-produit',
  templateUrl: 'commandes-edit-produit.html',
})
export class CommandesEditProduitPage {
  users = []
  $size = 15;
  $skip = 0;
  ...
  async getdetailsProduit(){
    ...
    await   this.cmdApi.getProduitcommandes_clients( this.prod_aj.NUMBON, this.prod_aj.ordre , this.serverURL).subscribe(
      resps => {
        this.data =resps;
        this.prod_aj = {
 ...
 async postProdDevis(devis$){
    await this.cmdApi.updateProduitcommandes_clients(devis$,this.serverURL)
      .subscribe(res => {
        ...
        alert.present();
        this.cmdData.refresh_list_DPRO();
       // this.navCtrl.push(CommandesDataPage, devis$.NUMBON);
      }, (err) => { console.log(err); });
  }
