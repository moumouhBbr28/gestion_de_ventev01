...
import {DevisEditProduitPage} from "../devis-edit-produit/devis-edit-produit";
import { DevisRemisePage } from "../devis-remise/devis-remise";
...
@IonicPage()
@Component({
  selector: 'page-devis-data',
  templateUrl: 'devis-data.html',
})
export class DevisDataPage {
  ...
  totalHt=0;totalNetHt=0;
  tauxRemise=0;taux_tva=0;
  totalTTC=0;dprod ;
  totaux_netht=0;
  serverURL;
 ...
 infoProformaGle() {
  this.devisApi.getTypeVentSelDevis(this.numBon ,  this.serverURL).subscribe(
      res => {
        this.data = res;
        ...
        //this.mod_dev.DATEBON =  (moment( new Date(this.proforma_data.DATE_BON).toISOString()).format('DD/MM/YYYY'));
           ...
           this.devisApi.getTypeVentSelDevis(this.numBon ,  this.serverURL).subscribe(
             res => {
               ...
               if(this.valide_bdd_r == "OUI"){
                 this.devisApi.devalider_devi(this.numBon ,  this.serverURL).subscribe(
                   res => {
                     let alert = this.alertCtrl.create({
                       title: "Information",
                       subTitle: res ['msg'],
                 ...
                 this.devisApi.valider_devis(this.numBon ,  this.serverURL).subscribe(
                   resp => {
                     let alert = this.alertCtrl.create({
                       title: "Information",
                       subTitle: resp['msg'],
 ...
 editer ($NUMBON,$LIGNE,$ORDRE,$VALIDE){
    if($VALIDE){
       let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas editer ce produit parce que la proforma est validée!',
        buttons: ['OK']
      });
      alert.present();
      ...
      this.navCtrl.push(DevisEditProduitPage, this.dprod);
  ...
  ajouter_des_prod_d ($NUMBON,$VALIDE) {
    if($VALIDE){
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas ajouter des produits parce que la proforma est validée !',
        buttons: ['OK']
      });
      alert.present();
    } else{
      this.navCtrl.push(DevisAjoutProduitPage, $NUMBON);
    }
  };
 ...
 data_remise = {
  ...
  ajt_remise_globle($c_proforma,$totaux_bht,$valide){
   if($valide){
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas appliquer une remise globale parce que la proforma est validée !',
        buttons: ['OK']
      });
      alert.present();
    } else{
    ...
    this.navCtrl.push(DevisRemisePage, this.data_remise);
    }
  };
  ...
  infoProduitOrdre() {
    ...
    this.devisApi.get_d_proforma_ParNumBon(this.numBon ,  this.serverURL).subscribe(
      res => {
        this.data = res;
  ...
  calcul_rem_details() {
    ...
    this.devisApi.get_detail_totaux(this.numBon, this.serverURL).subscribe(
      res => {
        this.data = res;
  ...
  supprimer($NUMBON,$ORDRE,$VALIDE){
    if($VALIDE){
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas supprimer ce produit parce que la proforma est validée !",
        buttons: ['Annuler']
      });
      alert.present();
        ...
        buttons: [
            ...
            role: 'cancel' },
          {
            ...
            handler: () => {
              ...
              this.devisApi.delete_produit_devis($NUMBON,$ORDRE, this.serverURL).subscribe(
                res => {
                  this.data = res;
                  this.refresh_list_DPRO();
                  let alert = this.alertCtrl.create({
  ...
  modifier_devis($mod_dev){
    ...
    if($mod_dev.VALIDE){
      ...
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas modifier une proforma validée !",
        buttons: ['OK']
      });
      alert.present();
      ...
      this.devisApi.update_info_devis($mod_dev ,  this.serverURL).subscribe(
        res => {
          this.data = res;
          let alert = this.alertCtrl.create({
            title: "Information",
