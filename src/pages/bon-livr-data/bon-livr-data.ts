...
import { BonLivrRemisePage } from "../bon-livr-remise/bon-livr-remise";
import { ReglementsPage } from "../reglements/reglements";
import { Storage } from '@ionic/storage';
import { BonLivrTotauxPage } from '../bon-livr-totaux/bon-livr-totaux';
...
@IonicPage()
@Component({
  selector: 'page-bon-livr-data',
  templateUrl: 'bon-livr-data.html',
})
export class BonLivrDataPage {
  $size = 15;
  $skip = 0; 
  mod_dev: Commande;
  data: any;
  ...
  infoProformaGle() {
    this.cmdApi.getTypeVentSelCmd(this.numBon, this.serverURL).subscribe(
      res => {
        ...
        this.mod_dev.typevente_pardefaut = this.proforma_data.TYPEVENTES;
        ...
        this.mod_dev.refPiece = this.proforma_data.NUMCDE;
        this.NUMFACT = this.proforma_data.NUMFACT;
  ...
  editer($NUMBON,$LIGNE, $ORDRE, $VALIDE) {
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas editer ce produit parce que le bon est validé !',
        buttons: ['OK']
      });
      alert.present();
      ...
      this.navCtrl.push(BonLivrEditProduitPage, this.dprod);
    }
  };
  ...
  ajouter_des_prod_d($NUMBON, $VALIDE) {
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas ajouter des produits parce que le bon est validé!',
        buttons: ['OK']
      });
      alert.present();
    } else {
      this.navCtrl.push(BonLivrAjoutProduitPage, $NUMBON);
      this.refresh_list_DPRO();
    }
  ...
  ajt_remise_globle($c_proforma, $totaux_bht, $valide) {
    if ($valide) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: 'On ne peut pas appliquer une remise globale parce que le bon est validé !',
        buttons: ['OK']
      });
      alert.present();
    } else {
      ...
      this.navCtrl.push(BonLivrRemisePage, this.data_remise);
    }
  };
  ...
  //   alert.present();
  // };
  goTodetailGlobal($numBon) {
    let profileModal = this.modalCtrl.create(BonLivrTotauxPage, { numBon: $numBon }, {
      cssClass: "my-modal"
    });
    profileModal.present();
    //  this.navCtrl.push(BonLivrTotauxPage,{numBon:$numBon});
  ...
  supprimer($NUMBON, $ORDRE, $VALIDE) {
    if ($VALIDE) {
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas supprimer ce produit parce que le bon est validé !",
        buttons: ['Annuler']
      });
      alert.present();
  ...
  modifier_devis($mod_dev) {
    ...
    if ($mod_dev.VALIDE) {
      ...
      let alert = this.alertCtrl.create({
        title: "Information",
        subTitle: "On ne peut pas modifier un bon validé !",
        buttons: ['OK']
      });
      alert.present();
